...
from .errors import ChecksumMismatchError, NetworkFailureError, CancellationError

class DownloadWorker:
    def __init__(self, transfer: TransferEngine, verification: VerificationEngine, 
                 staging: StagingArea, progress: ProgressReporter, metrics: MetricsCollector):
    ...
    def execute(self, request: DownloadRequest) -> DownloadResult:
        ...
        try:
            ...
            self.progress.report(ProgressEvent(id=request.id, status="verifying", bytes_downloaded=bytes_downloaded, total_bytes=bytes_downloaded))
            ...
            self.progress.report(ProgressEvent(id=request.id, status="completed", bytes_downloaded=bytes_downloaded, total_bytes=bytes_downloaded))
            ...
            return DownloadResult(id=request.id, component_slug=request.component_slug, staged_path="", bytes_downloaded=0, success=False, error_message=str(e))

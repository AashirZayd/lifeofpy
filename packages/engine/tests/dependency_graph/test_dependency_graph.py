...
from engine.dependency_graph.errors import CycleDetectedError
...
def test_successful_resolution(service):
    available_nodes = {
        "button": DependencyNode(id="button", version="1.0.0", frameworks=["customtkinter"], dependencies=["label", "icon"]),
...
def test_framework_mismatch(service):
    available_nodes = {
        "button": DependencyNode(id="button", version="1.0.0", frameworks=["tkinter"], dependencies=[])
    }

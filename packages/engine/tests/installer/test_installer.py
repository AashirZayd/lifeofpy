...
from engine.installer.errors import InstallError, ProjectLockedError
from engine.installer.events import EventBus
    ...
    def validate_component(self, path: Path):
        ...
        if "fail_verify" in str(path):
            class FailedReport:
                has_errors = True
                def get_errors(self):
                    class Err: description = "Bad manifest"
                    return [Err()]
            return FailedReport()
...
def test_successful_installation(service, tmp_path):
    plan = InstallPlan(
        total_components=1,
        stages=[InstallPlanStage(name="Download", components=["button"])]
    )
...
def test_download_failure_triggers_rollback(service, tmp_path):
    plan = InstallPlan(
        total_components=1,
        stages=[InstallPlanStage(name="Download", components=["fail_download"])]
    )
...
def test_verification_failure_triggers_rollback(service, tmp_path):
    plan = InstallPlan(
        total_components=1,
        stages=[InstallPlanStage(name="Download", components=["fail_verify"])]
    )

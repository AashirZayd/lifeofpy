...
class HttpClient:
    ...
    def request_json(self, url: str, headers: Optional[Dict[str, str]] = None) -> dict:
        ...
        try:
            ...
            raise NetworkError(f"Failed to decode JSON from {url}: {e}")
    ...
    def request_bytes(self, url: str, headers: Optional[Dict[str, str]] = None) -> bytes:
        ...
        for attempt in range(self.retries):
            try:
                ...
                if e.code == 404:
                    raise RegistryUnavailableError(f"Resource not found: {url}")
                last_exception = e
        ...
        raise NetworkError(f"Failed to fetch {url} after {self.retries} attempts. Last error: {last_exception}")

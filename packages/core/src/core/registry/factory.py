    ...
    def create_provider(self, config: dict) -> RegistryProviderProtocol:
        ...
        if not registry_url:
            registry_url = os.getenv(
                "LIFEOFPY_REGISTRY",
                "https://raw.githubusercontent.com/lifeofpy/registry/main/registry/v1/",
            )
        ...
        if registry_url.startswith("http://") or registry_url.startswith("https://"):
            ...
            registry_url.startswith("file://")
            or registry_url.startswith("/")
            or registry_url.startswith("C:\\")
            or registry_url.startswith("D:\\")
        ):
